...
import ipron.cloud.web.advice.exception.*;
import ipron.cloud.web.model.dto.ErrorCode;
import ipron.cloud.web.model.response.ErrorResult;
...
public class ExceptionAdvice {
    ...
    /**
     * Mongodb command 실패시 처리
     */
    @ExceptionHandler(MongoWriteException.class)
    public ResponseEntity<ErrorResult> handleHttpException(final MongoWriteException e) {
        final ErrorResult response = ErrorResult.of(ErrorCode.DB_EXECUTE_ERROR);
        if(response.getMsg().isEmpty()) {
            response.setMsg(e.getMessage());
        }
        return new ResponseEntity<>(response, HttpStatus.INTERNAL_SERVER_ERROR);
    ...
    public ResponseEntity<ErrorResult> handleHttpException(final IllegalArgumentException e) {
        final ErrorResult response = ErrorResult.of(ErrorCode.INTERNAL_SERVER_ERROR);
        if(response.getMsg().isEmpty()) {
            response.setMsg(e.getMessage());
        }
    ...
    public ResponseEntity<ErrorResult> handleHttpException(final Exception e) {
        ...
        if(response.getMsg().isEmpty()) {
            response.setMsg(e.getMessage() != null ? e.getMessage() : e.getCause().getMessage());
        }
        return new ResponseEntity<>(response, HttpStatus.INTERNAL_SERVER_ERROR);
    }

    public ResponseEntity<ErrorResult> handleHttpException(final Throwable e) {
        ErrorResult response = ErrorResult.of(ErrorCode.UNKNOWN_ERROR);
        if(response.getMsg().isEmpty()) {
            response.setMsg(e.getMessage() != null ? e.getMessage() : e.getCause().getMessage());
        }
