package ipron.cloud.web.importer;
...
@Slf4j
public class YamlPropertiesEnvironmentPostProcessor implements EnvironmentPostProcessor, Ordered {
        ...
        for(Resource resource : resources) {
            loadYaml(resource, environment)
                    .forEach(propertySource -> {
                        ...
                        if(propertySourceName.matches("application-.*")){
                            String active = environment.getProperty(SPRING_PROFILES+".active");
                            if(active != null && propertySourceName.contains(active)){
