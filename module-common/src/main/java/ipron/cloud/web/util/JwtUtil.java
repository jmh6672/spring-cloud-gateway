...
//    public final static long TOKEN_VALIDATION_SECOND = 1000L  * 60 * 60;  //60 minute
    ...
    /**
     * JWT token 내 payload(body) 영역 중 _id를 return 한다.
     * @param token
     * @return
     */
    public String getUniqueidInToken(String token) {
        String _id = extractAllClaims(token).get("_id", String.class);
        return _id;
    }
    /**
     * JWT token 내 payload(body) 영역 중 tntId를 return 한다.
     * @param token
     * @return
     */
    public String getUniqueidInTntid(String token) {
        String _id = extractAllClaims(token).get("tntId", String.class);
        return _id;
    }
    ...
    /**
     * token의 상태를 boolean으로 return한다. JWT내 Payload의 _id와 만료일이 유효한지 확인한다.
     *
     * @param token
     * @param userMap
     * @return
     ...
     * @return
     */
    public Boolean invaildTokenCheck(String token, String request_id) {
        Boolean result = false;
        final String response_id = getUniqueidInToken(token);
        if (response_id.equals(request_id) && !isTokenExpired(token)) {
            result = true;
        }
        return result;
    }
